...
Raphael.colorwheel = function(target, color_wheel_size, no_segments) {
  ...
  function input(target) {
    ...
    $(target).keyup(function() {
      if (this.value.match(/^#([0-9A-F]){3}$|^#([0-9A-F]){6}$/gim)) {
  ...
  function events_setup() {
    ...
    $([bs_square.b.node, bs_square.cursor[0].node]).on(
      "mousedown touchstart",
      function(e) {
  ...
  function cursor_create(size) {
    var set = canvas
      .set()
      .push(
        canvas.circle(0, 0, size).attr({ "stroke-width": 5, stroke: "#333" }),
        canvas
          .circle(0, 0, size + 15)
          .attr({ "stroke-width": 3, stroke: "#FFF", opacity: 0.5 })
      );
  ...
  function hue_segement_shape() {
    ...
    return path.replace(/@H/gim, tri_size * 2).replace(/@W/gim, tri_size);
  }
  ...
  function create_hue_ring() {
    ...
    hue_ring.event = canvas
      .circle(center, center, center - tri_size - padding)
      .attr({ "stroke-width": tri_size * 2, opacity: 0.01 });

    hue_ring.outline = canvas
      .circle(center, center, center - tri_size - padding)
      .attr({ stroke: "#000", "stroke-width": tri_size * 2 + 3, opacity: 0.1 });
